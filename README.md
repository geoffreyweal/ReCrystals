...
It is recommended to read the installation page before using the ReCrystals program. See [Installation: Setting Up ReCrystals and Pre-Requisites Packages](https://geoffreyweal.github.io/ReCrystals/Installation) for more information. Note that ReCrystals is not published to PyPI or conda; install it from GitHub with ``pip3 install --upgrade --user git+https://github.com/geoffreyweal/ReCrystals.git``. 
