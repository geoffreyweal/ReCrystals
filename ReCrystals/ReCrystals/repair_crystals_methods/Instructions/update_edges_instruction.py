"""
update_edges_instruction.py, Geoffrey Weal, 11/4/24

This method is designed to update the bond information of edges between atoms in a molecule.
"""
...
from ReCrystals.ReCrystals.repair_crystals_methods.Modification_Methods.update_edges_to_molecule_graph     import update_edges_to_molecule_graph

def update_edges_instruction(instruction, molecules, molecule_graphs):
	"""
	This method is designed to update the bond information of edges between atoms in a molecule.
	...
	# Second, record all the information about the edges to update to the molecule
	edges_to_update = instruction['edges']
	...
	# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
	# Part II: Make sure that the variable above have been updated so they are ready for the "update_edges_to_molecule_graph" method.
	...
	if not (isinstance(edges_to_update,list) or isinstance(edges_to_update,tuple)):
	...
	edges_to_update = list(edges_to_update)
	...
	for index, edge_input in enumerate(edges_to_update):
		...
		if not len(edge_input) == 3:
			to_string  = f'Error: the number of values in the "edges[{index}]" tuple/list is 3.\n'
		...
		# 10.5: Check that edge_input[2] is a dictionary
	...
	# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
	# PART III: Run the "update_edges_to_molecule_graph" method.

	# Twelfth, update the edge information to the molecule graph (for the associated molecule). 
	update_edges_to_molecule_graph(molecule_name, edges_to_update, molecule_graphs)
