"""
add_edges_instruction.py, Geoffrey Weal, 11/4/24

This method is designed to add edges between atoms in a molecule that are missing. This is equivalent to declaring there are bonds between atoms in the molecule. 
"""
...
from ReCrystals.ReCrystals.repair_crystals_methods.Modification_Methods.add_edges_to_molecule_graph        import add_edges_to_molecule_graph

def add_edges_instruction(instruction, molecules, molecule_graphs):
	"""
	This method is designed to add edges between atoms in a molecule that are missing. 

	This is equivalent to declaring there are bonds between atoms in the molecule. 
	...
	# Second, record all the information about the edges to add to the molecule
	edges_to_add = instruction['edges']
	...
	# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
	# Part II: Make sure that the variable above have been updated so they are ready for the "add_edges_to_molecule_graph" method.
	...
	if not (isinstance(edges_to_add,list) or isinstance(edges_to_add,tuple)):
	...
	for index, edge_input in enumerate(edges_to_add):
		...
		# 10.2: Make sure that edge_input is only a size of 2 or 3
		if len(edge_input) not in [2, 3]:
			to_string  = f'Error: the number of values in the "edges[{index}]" tuple/list is not 2 or 3.\n'
		...
		# 10.5: Make sure that the bond information exists, if not add empty bond_info dictionary to edge_input
		if len(edge_input) == 3:

			# 10.5.1: Check that edge_input[2] is a dictionary
			if not isinstance(edge_input[2], dict):
				to_string  = f'Error: The bond information for instruction["edges"][{index}] is not a dictionary.\n'
				to_string += f'Problematic atom index input: {edge_input[ii]}\n'
				to_string += f'Edge input with problem: {edge_input}\n'
				to_string += f'Full Instruction: {instruction}\n'
				to_string += 'Check this.'
			...
			# 11.5.2: Add the empty bond information dictionary to edge_input
			edge_input.append({})

	# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
	# PART III: Run the "add_edges_to_molecule_graph" method.

	# Eleventh, add the edge information to the molecule graph (for the associated molecule). 
	add_edges_to_molecule_graph(molecule_name, edges_to_add, molecule_graphs)
